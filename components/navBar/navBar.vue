<template>
	<view class="nav">
		...
		<view v-if="isHome" class="headNav"
			...
			<text class="city">位置地区</text>
			<view style="flex: 1;">
				<navigator url="/pages/search/index" :style="'height:'+menu.height*2+'rpx;line-height:'+menu.height*2+'rpx;margin:top:'+(menu.top*2) - status +'rpx;margin-left:32rpx;margin-right:'+(menu.width*2+24)+
				'rpx;background:#f4f4f4;border-radius:200rpx;text-align:center;'">
					<text class="search-text">搜索想要的</text>
				</navigator>
			</view>
		</view>
		<view v-else class="navbar" :style="'height:' + navHeight + 'rpx;' + containerStyle">
			...
			<view class="nav-title" v-if="titleText">
				<view :style="'height:'+navHeight+'prx;line-height:'+navHeight+'rpx;'+textStyle">{{titleText}}</view>
			</view>
		</view>
	</view>
</template>
